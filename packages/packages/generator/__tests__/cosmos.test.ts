  ...
  describe('File Generation', () => {
    ...
    describe('YAML Content Generation Snapshots', () => {
      ...
      it('should generate all files for multiple chains - snapshots', () => {
        ...
        expect(
          existsSync(join(outputPath, 'cosmoshub', 'configmap.yaml'))
        ).toBe(true);
        ...
        expect(
          existsSync(join(outputPath, 'cosmoshub', 'validator.yaml'))
        ).toBe(true);
        ...
        expect(osmosisConfigMapYaml).toMatchSnapshot(
          'multiple-chains-osmosis-configmap-yaml'
        );
        expect(cosmoshubConfigMapYaml).toMatchSnapshot(
          'multiple-chains-cosmoshub-configmap-yaml'
        );
      });
      ...
      it('should handle custom chain YAML generation - snapshots', () => {
        ...
        const configMapYaml = readFileSync(
          join(outputPath, 'custom', 'configmap.yaml'),
          'utf-8'
        );
        const serviceYaml = readFileSync(
        ...
        const genesisYaml = readFileSync(
          join(outputPath, 'custom', 'genesis.yaml'),
          'utf-8'
        );

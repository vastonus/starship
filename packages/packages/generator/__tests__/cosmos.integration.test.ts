...
describe('Cosmos Integration Tests', () => {
  ...
  describe('End-to-End Generation with BuilderManager', () => {
    it('should generate complete single-chain setup using BuilderManager', () => {
      ...
      const manager = new BuilderManager(processedConfig);
      
      // Build manifests to specific test subdirectory
      const testSubDir = join(testOutputDir, 'single-chain-manager');
      manager.build(testSubDir);
      
      // Verify files were generated by checking if the output directory has YAML files (including subdirectories)
      ...
      // Test the builder directly to get manifests for verification
      const cosmosBuilder = new CosmosBuilder(processedConfig);
      const manifests = cosmosBuilder.buildManifests();
      const cosmosManifests = manifests.filter((m: any) =>
        m.metadata?.name?.includes('osmosis')
      );
      
      expect(cosmosManifests.length).toBeGreaterThan(0);
      
      // Snapshot test for end-to-end generation
      expect({
        manifestCount: cosmosManifests.length,
        fileCount: files.length,
        testDirectory: 'single-chain-manager',
        hasGenesisService: cosmosManifests.some((m: any) =>
          m.kind === 'Service' && m.metadata?.name?.includes('genesis')
        ),
        hasStatefulSet: cosmosManifests.some((m: any) =>
          m.kind === 'StatefulSet'
        ),
        hasConfigMaps: cosmosManifests.some((m: any) =>
          m.kind === 'ConfigMap'
        )
    ...
    it('should generate complete multi-chain setup using BuilderManager', () => {
      ...
      // Build manifests to specific test subdirectory
      const testSubDir = join(testOutputDir, 'multi-chain-manager');
      manager.build(testSubDir);
      
      // Verify files were generated (including subdirectories)
      const getAllYamlFiles = (dir: string): string[] => {
        const files: string[] = [];
        const items = readdirSync(dir, { withFileTypes: true });
        for (const item of items) {
      ...
      // Snapshot test for multi-chain generation
      expect({
        totalManifests: cosmosManifests.length,
        fileCount: files.length,
        testDirectory: 'multi-chain-manager',
        osmosisManifests: cosmosManifests.filter((m: any) =>
          m.metadata?.name?.includes('osmosis')
        ).length,
        cosmoshubManifests: cosmosManifests.filter((m: any) =>
          m.metadata?.name?.includes('cosmoshub')
        ).length
